...
foreach $exp (keys %exps) {
  ...
  foreach $resam ('stratified', 'metropolis') {
    open(FILE, ">results/time_${resam}_${exp}.csv") || die;
    for ($i = 1; $i <= 10; ++$i) {
      $P = $i*1024;
      print "P=$P...\n";
      print FILE "$P\t";
      print FILE `build/filter -P $P -T $T -L $L -h $H --seed $SEED --resampler $resam --min-ess $MIN_ESS --init-file $INIT_FILE --force-file $FORCE_FILE --obs-file $OBS_FILE --init-ns $INIT_NS --force-ns $FORCE_NS --obs-ns $OBS_NS --output-file $OUTPUT_FILE --output $OUTPUT --time $TIME`;
    }
    close FILE;
  }
}
